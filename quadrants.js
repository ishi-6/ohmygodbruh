...
function filterBlipsByType(type) {
...
function showBlipGroup(blipGroup) {
  blipGroup.style.opacity = '1'
  blipGroup.style.visibility = 'visible'
  blipGroup.style.pointerEvents = 'auto'
  blipGroup.style.transition = 'opacity 0.3s ease-in-out'
}

function showBlipListItem(blipListItem) {
  blipListItem.style.opacity = '1'
  blipListItem.style.display = ''
}
...
function selectRadarQuadrant(order, startAngle, name) {
  ...
  // Type filter UI logic for platforms quadrant
  if (order === 'second') {
    showPlatformTypeFilter()
    filterBlipsByType('all types')
  } else {
    hidePlatformTypeFilter()
    // Reset all blips and descriptions to visible when leaving platforms quadrant
    const blips = document.querySelectorAll('.quadrant-group-second .blip-link')
    blips.forEach((blip) => {
      ...
      const blipId = blip.getAttribute('data-blip-id')
      const blipListItem = document.querySelector(`.blip-list__item-container[data-blip-id="${blipId}"]`)
      if (blipListItem) {
        blipListItem.style.opacity = '1'
        blipListItem.style.display = ''
      }
...
function renderPlatformTypeFilter() {
  const typeFilterId = 'platform-type-filter'
  const typeOptions = ['all types', 'service', 'compute', 'networking', 'database', 'storage']

  let filterContainer = document.getElementById(typeFilterId)
  if (filterContainer) return

  const radarHeader = d3.select('main .graph-header')
  filterContainer = radarHeader
    .append('div')
    .attr('id', typeFilterId)
    .classed('platform-type-filter', true)
    .style('opacity', '0')
    .style('pointer-events', 'none')

  const buttonsDiv = filterContainer.append('div').classed('filter-buttons', true)
  ...
  d3.select('.filter-button[data-type="all types"]').classed('active', true)
}

function showPlatformTypeFilter() {
  const filterContainer = document.getElementById('platform-type-filter')
  if (filterContainer) {
    d3.select(filterContainer).style('opacity', '1').style('pointer-events', 'auto')
  }
}

function hidePlatformTypeFilter() {
  const filterContainer = document.getElementById('platform-type-filter')
  if (filterContainer) {
    d3.select(filterContainer).style('opacity', '0').style('pointer-events', 'none')
  }
}
